...
class ResetPasswordNotification extends Notification implements ShouldQueue
{
    ...
    {
        $url = url(route('web.admin.password.reset', [
            'token' => $this->token,
        ...
        return (new MailMessage)
            ...
            ->greeting(__('notifications.reset_pass.greeting', ['Nick' => $notifiable->name]))
            ->line(__('notifications.reset_pass.message01'))
            ->action(__('notifications.reset_pass.button'), $url)
            ->line(__('notifications.reset_pass.message02', ['count' => config('auth.passwords.admins.expire')]));
    }
}
